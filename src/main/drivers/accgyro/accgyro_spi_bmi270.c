...
#define BMI270_REG_PWR_CTRL_GYR_ACC 0x0E
...
static int16_t BMI270_do_foc(const busDevice_t *bus);


static uint8_t read_reg(const busDevice_t *bus, uint8_t reg)
{
	// BMI270 needs a dummy byte when reading regsiters
	uint8_t data[2];

	spiBusReadRegisterBuffer(bus, reg, data, 2);
...
uint8_t bmi270Detect(const busDevice_t *bus)
{
    ...
    /* Read CHIP ID to activate SPI (see p. 139) */
    read_reg(bus, BMI270_REG_CHIPID);
    delay(10); // Give SPI some time to start up

    /* Check the chip ID */
    uint8_t chip_id = read_reg(bus, BMI270_REG_CHIPID);
    if (chip_id != 0x24) {
        return MPU_NONE;
    }
...
//    if (bfOsdConfig()->bmi270foc) {
//        int16_t foc_ret = BMI270_do_foc(bus);
//        bfOsdConfigMutable()->bmi270foc = false;
//        bfOsdConfigMutable()->bmi270foc_ret = foc_ret;
//        writeEEPROM();
//    }
//#endif
...
static int32_t upload_config_file(const busDevice_t *bus)
{
	uint16_t bytes_sent = 0;
	uint8_t status;
	const uint8_t * p_config = bmi270_config_file;

	// Disable advanced power save mode
	spiBusWriteRegister(bus, BMI270_REG_PWR_CONF, 0x00);
	delay(2);

	// Write INIT_CTRL.init_ctrl=0x00 to prepare config load
	spiBusWriteRegister(bus, BMI270_REG_INIT_CTRL, 0x00);

	// Send the data
    IOLo(bus->busdev_u.spi.csnPin);
    spiTransferByte(bus->busdev_u.spi.instance, BMI270_REG_INIT_DATA);

    while (bytes_sent < sizeof(bmi270_config_file)) {
    	uint16_t n_send = MIN(1024U, sizeof(bmi270_config_file) - bytes_sent);
        spiTransfer(bus->busdev_u.spi.instance, p_config, NULL, n_send);
        bytes_sent += n_send;
        p_config += n_send;
    }
	...
	spiBusWriteRegister(bus, BMI270_REG_INIT_CTRL, 0x01);

	// Wait for internal status
	bool config_load_ok = false;
	for (uint8_t i=0; i < 30; i++) {
		status = read_reg(bus, BMI270_REG_INTERNAL_STATUS);
		if ((status & 0x0F) == 0x01) {
			config_load_ok = true;
			break;
		}
...
{
	// Reset
	//spiBusWriteRegister(bus, BMI270_REG_CMD, BMI270_CMD_SOFTRESET);
	//delay(100);

	// Upload config
	if (upload_config_file(bus) != 0) {
		return -1;
	}

	// Enable gyro and acc
	spiBusWriteRegister(bus, BMI270_REG_PWR_CTRL, BMI270_REG_PWR_CTRL_GYR_ACC);
	delay(2);

	// Set acc to performance mode, no averaging (acc_filter_perf = 1, acc_bwp = 0x02, acc_odr = 0xC)
	spiBusWriteRegister(bus, BMI270_REG_ACC_CONF, 0xAC);
	delay(2);
	...
	// Configure gyroscope: gyr_bwp = 0x02, gyr_noise_perf = 1, gyr_filter_perf = 1
	spiBusWriteRegister(bus, BMI270_REG_GYR_CONF, 0xE0 | BMI270_ODR_3200_Hz);
	delay(2);
	...
	spiBusWriteRegister(bus, BMI270_REG_PWR_CONF, 0x02);
	delay(2);

	// Configure INT1 pin
	spiBusWriteRegister(bus, BMI270_REG_INT1_IO_CTRL, 0x0A);
	delay(2);

	// Map data ready to INT1
    spiBusWriteRegister(bus, BMI270_REG_INT_MAP_DATA, 0x04);
    delay(2);
...
#if defined(USE_CHIBIOS)
FAST_CODE void bmi270ExtiHandler(extiCallbackRec_t *cb)
{
    CH_IRQ_PROLOGUE();
...
FAST_CODE bool bmi270AccRead(accDev_t *acc)
{
    enum {
        IDX_REG = 0,
		DUMMY,
...
FAST_CODE bool bmi270GyroRead(gyroDev_t *gyro)
{
    enum {
        IDX_REG = 0,
		DUMMY,
